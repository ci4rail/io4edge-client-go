...
func readStream(c *anain.Client, channel int, quitChan chan bool, sampleChan chan entry, wg *sync.WaitGroup, sampleRate int) {
	t := uint64(0)
	n := uint64(0)
	for {
		select {
		...
		if err != nil {
			...
			for _, sample := range samples {
				if t != 0 {
					et := uint64(1000000 / sampleRate)
					if sample.Timestamp-t > et+150 || sample.Timestamp-t < et-150 {
						log.Errorf("ch %d: sample %d wrong dt %d", channel, n, sample.Timestamp-t)
					}
				}
				t = sample.Timestamp
				if sample.Value > 0.1 || sample.Value < -0.1 {
					log.Errorf("ch %d: sample %d wrong val %.3f", channel, n, sample.Value)
				}
...
func main() {
	...
	for channel := 0; channel < 2; channel++ {
		...
		go readStream(c, channel, quitChan, sampleChan, &wg, sampleRate)
		wg.Add(1)
	}
	start := time.Now()
	numSamples := 0
