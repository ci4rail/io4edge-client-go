...
// Package pbchannelclient provides the API for protobuf communication with io4edge devices
...
func NewClientFromSocketAddress(address string) (*Client, error) {
	return newClientFromSocketAddress(address, zeroconfservice.NewFuncInfoDefault(address))
}
...
func NewClientFromService(serviceAddr string, timeout time.Duration) (*Client, error) {
	instance, service, err := zeroconfservice.ParseInstanceAndService(serviceAddr)
	if err != nil {
		return nil, err
	}
	if timeout == 0 {
	...
	svcInfo, err := zeroconfservice.GetServiceInfo(instance, service, timeout)
	if err != nil {
		return nil, err
	}
...
func NewClientFromUniversalAddress(addrOrService string, service string, timeout time.Duration) (*Client, error) {
	...
	if _, _, err = iputil.NetAddressSplit(addrOrService); err == nil {
